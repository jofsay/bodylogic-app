...
function App() {
  ...
  const handlePDF = useCallback(() => { generarPDFPedido({ perfilUsuario, nombrePedido, productosSeleccionados, descuentoActual, totalUnidades: totales.totalUnidades, totalPuntos: totales.totalPuntos, totalPrecioPublico: totales.totalPrecioPublico, totalConDescuento, obtenerSubtotal, textoModo, estadoTexto: estado.texto }); }, [perfilUsuario, nombrePedido, productosSeleccionados, descuentoActual, totales, totalConDescuento, obtenerSubtotal, textoModo, estado.texto]);
  const handlePrint = useCallback(() => { imprimirFormulario({ perfilUsuario, nombrePedido, productosSeleccionados, descuentoActual, totalUnidades: totales.totalUnidades, totalPuntos: totales.totalPuntos, totalPrecioPublico: totales.totalPrecioPublico, totalConDescuento, obtenerSubtotal, subtitulo: textoModo, estadoTexto: estado.texto }); }, [perfilUsuario, nombrePedido, productosSeleccionados, descuentoActual, totales, totalConDescuento, obtenerSubtotal, textoModo, estado.texto]);
      ...
      <div style={{ maxWidth: "1500px", margin: "0 auto", position: "relative", zIndex: 1 }}>
        ...
        <SectionCard delay={1} key={`p-${animKey}`}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(220px,1fr))", gap: "12px" }}>
            ...
            <div style={cc}><label style={lb}>Buscar</label><div style={{ position: "relative" }}><input type="text" value={order.busqueda} onChange={(e) => order.setBusqueda(e.target.value)} placeholder="Ej. Omega 3, 4045156..." style={{ ...inp, paddingLeft: "36px" }} /><span style={{ position: "absolute", left: "11px", top: "50%", transform: "translateY(-50%)", fontSize: "15px", opacity: .35, pointerEvents: "none" }}>🔍</span></div></div><div style={cc}><label style={lb}>Nombre de quien hace el pedido</label><input type="text" value={nombrePedido} onChange={(e) => setNombrePedido(e.target.value)} placeholder="Ej. María Fernanda López" style={inp} /></div>
            <div style={{ ...ic, animation: "blScaleIn .35s ease both", animationDelay: ".1s" }}><div style={{ fontSize: "18px", fontWeight: 800, color: T.orange700, fontFamily: T.fontDisplay }}>{isD ? "Distribuidor" : isCP ? "Cliente Preferente" : isSim ? "Simulador" : "Ventas"}</div><div style={{ marginTop: "4px", color: T.textMuted, fontSize: "12px" }}>{isD ? "Ingreso y recompra" : isCP ? "Descuento progresivo" : isSim ? "Comparador de precios" : "Nota de venta al público"}</div></div>
          </div>
          ...
          {productosSeleccionados.length === 0 ? <div style={{ padding: "28px", borderRadius: T.r.lg, backgroundColor: "rgba(255,250,245,.6)", border: `2px dashed ${T.cream700}`, color: T.textMuted, textAlign: "center", fontSize: "14px" }}>Aún no has agregado productos.</div> : (
            ...
            {isSim ? (
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(130px,1fr))", gap: "8px", marginBottom: "14px" }}>
                ...
                <MiniDato label="Puntos internos" value={totales.totalPuntos} large />
                <MiniDato label="Total a pagar" value={formatoMoneda(totales.totalPrecioPublico)} highlight large />
              </div>
            ) : (
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(130px,1fr))", gap: "8px", marginBottom: "14px" }}>
                <MiniDato label="Unidades" value={totales.totalUnidades} highlight large />
            ...
            <div style={{ display: "grid", gap: "10px" }}>{productosSeleccionados.map((item, i) => {
              ...
              const descTotal = isVentas ? pubTotal : isSim ? pubTotal * (1 - descuentoSimulador / 100) : obtenerSubtotal(item);
              const descLabel = isVentas ? 0 : isSim ? descuentoSimulador : descuentoActual;
              return (
              <div key={item.codigo} className="bl-card" style={{ background: `linear-gradient(135deg,rgba(255,250,245,.8),rgba(255,244,234,.6))`, border: `1px solid ${T.cream500}`, borderRadius: T.r.lg, padding: "14px", animation: `blFadeUp .3s ease both`, animationDelay: `${i * .04}s` }}>
                ...
                <div style={{ marginTop: "10px", display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(120px,1fr))", gap: "6px" }}>
                  {isVentas ? (
                    <>
                      <MiniDato label="Precio unitario" value={formatoMoneda(item.precioPublico)} />
                      <MiniDato label="Unidades" value={item.unidades} />
        ...
        {/* ══ TABLA MAESTRA ══ */}
        <SectionCard delay={4}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: "10px", marginBottom: "14px" }}><div><h2 style={secTitle}>Tabla maestra</h2><p style={secSub}><strong>{filasCalculadas.length}</strong> producto(s){order.categoriaSeleccionada !== "TODAS" ? ` en "${order.categoriaSeleccionada}"` : ""}{isSim && ` — Simulación al ${descuentoSimulador}%`}{isVentas && " — Venta al público"}</p></div><div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}><Btn onClick={handlePDF} active style={{ fontSize: "13px", padding: "9px 16px" }}>PDF</Btn><Btn onClick={handlePrint} style={{ fontSize: "13px", padding: "9px 16px" }}>Imprimir</Btn></div></div>
          {esMovil && vistaMovil === "cards" ? (
            ...
            ["Cat.","Cód.","Producto","Contenido","Uds.","Precio unitario","Subtotal"],
            (item,idx)=><tr key={item.codigo} ref={(el)=>{order.productRefs.current[item.codigo]=el;}} onClick={()=>order.setFilaActiva(item.codigo)} style={{backgroundColor:rowBg(item,idx),cursor:"pointer"}}><td style={tdS}>{item.categoria}</td><td style={tdS}>{item.codigo}</td><td style={{...tdS,color:T.textDark,fontWeight:600}}>{item.producto}</td><td style={tdS}>{item.contenido}</td><td style={tdS}><input type="number" min="0" value={item.unidades} onChange={(e)=>order.cambiarCantidad(item.codigo,e.target.value)} style={inpT}/></td><td style={tdS}>{formatoMoneda(item.precioPublico)}</td><td style={{...tdS,fontWeight:600,color:T.orange700}}>{formatoMoneda(item.subtotalPrecioPublico)}</td></tr>,
            ()=><tr style={{background:`linear-gradient(180deg,${T.cream100},${T.cream300})`}}><td style={tdT}/><td style={tdT}/><td style={{...tdT,fontWeight:700}}>TOTAL</td><td style={tdT}/><td style={{...tdT,fontWeight:700}}>{totales.totalUnidades}</td><td style={tdT}/><td style={{...tdT,fontWeight:700,color:T.orange700}}>{formatoMoneda(totales.totalPrecioPublico)}</td></tr>
          ) : isSim ? renderTable(
          ...
          <div className="bl-float-glass" style={{position:"fixed",left:"6px",right:"6px",bottom:"6px",zIndex:999,borderRadius:"18px",padding:"14px 16px",backgroundColor:isSim?`${simEstado.colorFondo}ee`:`${estado.colorFondo}ee`,border:`1.5px solid ${isSim?simEstado.colorBorde:estado.colorBorde}`,boxShadow:`0 -8px 40px rgba(0,0,0,.20),inset 0 1px 0 rgba(255,255,255,.3)`,color:isSim?simEstado.colorTexto:estado.colorTexto,transition:"all .35s cubic-bezier(.22,.61,.36,1)",paddingBottom:"max(14px, env(safe-area-inset-bottom, 14px))"}}>
            ...
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:"8px"}}>
              <div style={{display:"flex",gap:"14px",alignItems:"center",flexWrap:"wrap"}}>
                ...
                :isVentas?(<><FS l="Venta" v={totales.totalUnidades} c={estado.colorTexto}/><FS l="Pts int." v={totales.totalPuntos} c={estado.colorTexto} big/><FS l="Total" v={formatoMoneda(totales.totalPrecioPublico)} c={estado.colorTexto}/></>)
              ...
              <div className="bl-expand">
                <div style={{margin:"10px 0 8px",padding:"10px 14px",borderRadius:"12px",backgroundColor:"rgba(255,255,255,.35)",position:"relative",overflow:"hidden",backdropFilter:"blur(4px)"}}>
                  ...
                  <div style={{paddingLeft:"8px"}}>
                    <div style={{fontSize:"17px",fontWeight:800,lineHeight:1.15,color:isSim?simEstado.colorTexto:estado.colorTexto,fontFamily:T.fontDisplay}}>{isVentas?"Venta al público":isSim?`Simulador — ${descuentoSimulador}%`:isCP?"Cliente Preferente":modo==="compraInicial"?paqueteActual.nombre:modalidadLabel}</div>
                    <div style={{marginTop:"3px",fontSize:"12px",fontWeight:700,color:isSim?simEstado.colorTexto:estado.colorTexto,opacity:.80}}>{isVentas ? `Total a pagar: ${formatoMoneda(totales.totalPrecioPublico)}` : `Descuento: ${isSim?descuentoSimulador:descuentoActual}%`}</div>
                  </div>
                </div>
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:"6px",marginBottom:"8px"}}>
                  {isSim?(<>
                    ...
                    <FC l="Pts int." v={totales.totalPuntos} num e={estado}/>
                    <FC l="Unidades" v={totales.totalUnidades} e={estado}/>
                    <FC l="Total" v={formatoMoneda(totales.totalPrecioPublico)} e={estado}/>
                  </>):(<>
