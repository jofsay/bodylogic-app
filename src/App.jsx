      ...
      <div style={{ maxWidth: "1500px", margin: "0 auto", position: "relative", zIndex: 1 }}>
        ...
        <SectionCard delay={1} key={`p-${animKey}`}>
          ...
          {isSim && (
            <div key="sim" style={{ animation: "blFadeUp .3s ease both" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(220px,1fr))", gap: "12px" }}>
                <div style={cc}><label style={lb}>Descuento a simular</label><select value={descuentoSimulador} onChange={(e) => setDescuentoSimulador(Number(e.target.value))} style={sel}>{DESCUENTOS_SIMULADOR.map((d) => <option key={d} value={d}>{d === 0 ? "Precio público" : `${d}%`}</option>)}</select></div>
                <div style={{ ...ic, animation: "blScaleIn .3s ease both" }}><div style={{ fontSize: "28px", fontWeight: 800, color: T.orange600 }}>{descuentoSimulador}%</div><div style={{ marginTop: "3px", color: T.textMuted, fontSize: "12px" }}>Descuento seleccionado</div></div>
              </div>
            </div>
          )}
        ...
        <SectionCard delay={3}>
          ...
          {productosSeleccionados.length === 0 ? <div style={{ padding: "28px", borderRadius: T.r.lg, backgroundColor: "rgba(255,250,245,.6)", border: `2px dashed ${T.cream700}`, color: T.textMuted, textAlign: "center", fontSize: "14px" }}>Aún no has agregado productos.</div> : (
            ...
            {/* Simulador: show all discounts in order summary */}
            {isSim && <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(100px,1fr))", gap: "6px", marginBottom: "14px" }}>{DESCUENTOS_SIMULADOR.filter(d => d > 0).map(d => <MiniDato key={d} label={`${d}%`} value={formatoMoneda(productosSeleccionados.reduce((a, i) => a + i.unidades * calcularPrecioSimulador(i.precioPublico, d), 0))} />)}</div>}
            <div style={{ display: "grid", gap: "10px" }}>{productosSeleccionados.map((item, i) => (
              <div key={item.codigo} className="bl-card" style={{ background: `linear-gradient(135deg,rgba(255,250,245,.8),rgba(255,244,234,.6))`, border: `1px solid ${T.cream500}`, borderRadius: T.r.lg, padding: "14px", animation: `blFadeUp .3s ease both`, animationDelay: `${i * .04}s` }}>
                ...
                <div style={{ marginTop: "10px", display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(120px,1fr))", gap: "6px" }}>
                  ...
                  <MiniDato label={`Con ${descuentoActual}%`} value={formatoMoneda(obtenerSubtotal(item))} highlight />
                </div>
              </div>
            ))}</div></>
          )}
            ...
            <div style={{ display: "grid", gap: "10px" }}>{filasCalculadas.map((item, i) => { const act = order.filaActiva === item.codigo; return (
              <div key={item.codigo} ref={(el) => { order.productRefs.current[item.codigo] = el; }} className="bl-card" onClick={() => order.setFilaActiva(item.codigo)} style={{ background: item.unidades > 0 ? `linear-gradient(135deg,${T.orange50},${T.orange100})` : `linear-gradient(135deg,rgba(255,250,245,.7),rgba(255,244,234,.5))`, border: act ? `2px solid ${T.orange500}` : `1px solid ${T.cream500}`, borderRadius: T.r.lg, padding: "14px", boxShadow: act ? T.s.glow : T.s.xs, animation: `blFadeUp .3s ease both`, animationDelay: `${i * .025}s` }}>
                ...
                <div style={{ marginTop: "10px", display: "grid", gridTemplateColumns: "1fr 1fr", gap: "6px" }}>
                  ...
                  <MiniDato label={`Con ${descuentoActual}%`} value={formatoMoneda(obtenerPrecio(item))} highlight />
                </div>
              </div>); })}</div>
          ) : isSim ? renderTable(
            ["Cat.","Cód.","Producto","Uds.","Público",...DESCUENTOS_SIMULADOR.filter(d=>d>0).map(d=>`${d}%`)],
            (item,idx)=><tr key={item.codigo} ref={(el)=>{order.productRefs.current[item.codigo]=el;}} onClick={()=>order.setFilaActiva(item.codigo)} style={{backgroundColor:rowBg(item,idx),cursor:"pointer"}}><td style={tdS}>{item.categoria}</td><td style={tdS}>{item.codigo}</td><td style={{...tdS,color:T.textDark,fontWeight:600}}>{item.producto}</td><td style={tdS}><input type="number" min="0" value={item.unidades} onChange={(e)=>order.cambiarCantidad(item.codigo,e.target.value)} style={inpT}/></td><td style={tdS}>{formatoMoneda(item.precioPublico)}</td>{DESCUENTOS_SIMULADOR.filter(d=>d>0).map(d=><td key={d} style={tdS}>{formatoMoneda(calcularPrecioSimulador(item.precioPublico,d))}</td>)}</tr>,
            ()=><tr style={{background:`linear-gradient(180deg,${T.cream100},${T.cream300})`}}><td style={tdT}/><td style={tdT}/><td style={{...tdT,fontWeight:700}}>TOTAL</td><td style={{...tdT,fontWeight:700}}>{totales.totalUnidades}</td><td style={{...tdT,fontWeight:700}}>{formatoMoneda(totales.totalPrecioPublico)}</td>{DESCUENTOS_SIMULADOR.filter(d=>d>0).map(d=><td key={d} style={{...tdT,fontWeight:700}}>{formatoMoneda(filasCalculadas.reduce((a,i)=>a+i.unidades*calcularPrecioSimulador(i.precioPublico,d),0))}</td>)}</tr>
          ) : isCP ? renderTable(
          ...
          <div className="bl-float-glass" style={{position:"fixed",left:"6px",right:"6px",bottom:"6px",zIndex:999,borderRadius:"18px",padding:"14px 16px",backgroundColor:`${estado.colorFondo}ee`,border:`1.5px solid ${estado.colorBorde}`,boxShadow:`0 -8px 40px rgba(0,0,0,.20),inset 0 1px 0 rgba(255,255,255,.3)`,color:estado.colorTexto,transition:"all .35s cubic-bezier(.22,.61,.36,1)",paddingBottom:"max(14px, env(safe-area-inset-bottom, 14px))"}}>
            ...
            {!resumenContraido&&(
              <div className="bl-expand">
                <div style={{margin:"10px 0 8px",padding:"10px 14px",borderRadius:"12px",backgroundColor:"rgba(255,255,255,.35)",position:"relative",overflow:"hidden",backdropFilter:"blur(4px)"}}><div style={{position:"absolute",left:0,top:0,bottom:0,width:"3px",background:`linear-gradient(180deg,${estado.colorSemaforo},${estado.colorBorde})`,borderRadius:"3px 0 0 3px"}}/><div style={{paddingLeft:"8px"}}><div style={{fontSize:"17px",fontWeight:800,lineHeight:1.15,color:estado.colorTexto,fontFamily:T.fontDisplay}}>{isCP?"Cliente Preferente":modo==="compraInicial"?paqueteActual.nombre:modalidadLabel}</div><div style={{marginTop:"3px",fontSize:"12px",fontWeight:700,color:estado.colorTexto,opacity:.80}}>Descuento: {descuentoActual}%</div></div></div>
